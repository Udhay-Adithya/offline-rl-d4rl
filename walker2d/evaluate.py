"""
Evaluation script for trained Walker2d offline RL models.

Usage:
    python -m walker2d.evaluate --algo cql --variant medium \
        --model_path ./results/walker2d/cql_medium_seed0/best_model.pt
    python -m walker2d.evaluate --algo iql --variant medium-expert \
...
def evaluate(
    ...
    """Evaluate a trained Walker2d model across multiple seeds."""
    ...
    print("\n" + "=" * 60)
    print(f"Walker2d-{variant} | {algo.upper()} | {len(seeds)} eval seeds")
...
def main():
    parser = argparse.ArgumentParser(
        description="Evaluate a trained offline RL model on Walker2d-v2"
    )
    parser.add_argument(
        "--algo",
        type=str,
