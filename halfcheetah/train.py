"""
Training script for HalfCheetah offline RL experiments.

Usage:
    python -m halfcheetah.train --algo cql --variant medium
    python -m halfcheetah.train --algo iql --variant medium-expert --seed 42
    python -m halfcheetah.train --algo bc --variant expert --total_steps 500000
"""
...
from halfcheetah.dataset import load_halfcheetah_dataset
...
def build_agent(algo, device, **override_kwargs):
    """Build an agent with HalfCheetah-tuned defaults, overridable by kwargs."""
...
def train(
    ...
    eval_episodes=10,
    save_freq=100_000,
    output_dir="./results/halfcheetah",
    device="auto",
    **algo_kwargs,
):
    """Train an offline RL agent on a HalfCheetah D4RL dataset."""
    np.random.seed(seed)
    torch.manual_seed(seed)
    device = _resolve_device(device)

    # Fall back to algorithm defaults if not specified.
    ...
    # Print expected reference score for this combo.
    expected = EXPECTED_SCORES.get(algo, {}).get(variant)
    if expected is not None:
        print(f"Reference normalized score ({algo}, {variant}): ~{expected:.1f}%")

    print(f"\nLoading HalfCheetah '{variant}' dataset...")
    dataset = load_halfcheetah_dataset(variant=variant, normalize_states=True)
    ...
    print(
        f"\nTraining {algo.upper()} for {total_steps:,} steps on halfcheetah-{variant}..."
    )
    best_score = -np.inf

    for step in tqdm(range(total_steps), desc=f"halfcheetah-{variant}/{algo}"):
    ...
    with open(os.path.join(save_dir, "final_metrics.txt"), "w", encoding="utf-8") as f:
        ...
        f.write(f"Environment: halfcheetah-{variant}\n")
        f.write(f"Seed: {seed}\n")
...
def main():
    parser = argparse.ArgumentParser(description="Train offline RL on HalfCheetah-v2")
    parser.add_argument(
        "--algo",
        type=str,
        default="cql",
    ...
    parser.add_argument("--output_dir", type=str, default="./results/halfcheetah")
    parser.add_argument(
