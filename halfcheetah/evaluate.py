"""
Evaluation script for trained HalfCheetah offline RL models.

Usage:
    python -m halfcheetah.evaluate --algo cql --variant medium \
        --model_path ./results/halfcheetah/cql_medium_seed0/best_model.pt
    python -m halfcheetah.evaluate --algo iql --variant medium-expert \
...
def evaluate(
    ...
    """Evaluate a trained HalfCheetah model across multiple seeds."""
    ...
    print("\n" + "=" * 60)
    print(f"HalfCheetah-{variant} | {algo.upper()} | {len(seeds)} eval seeds")
...
def main():
    parser = argparse.ArgumentParser(
        description="Evaluate a trained offline RL model on HalfCheetah-v2"
    )
    parser.add_argument(
        "--algo",
        type=str,
