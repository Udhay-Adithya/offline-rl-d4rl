"""
Hopper-specific dataset loader.

Wraps the common D4RLDataset with Hopper-specific validation,
...
class HopperDataset(D4RLDataset):
    """Dataset loader specialized for Hopper-v2 environments."""
    ...
    def __init__(
        ...
        if variant not in DATASET_VARIANTS:
            raise ValueError(
                f"Unknown Hopper variant '{variant}'. "
        ...
        # Validate dimensions match expected Hopper specs.
        assert (
...
def load_hopper_dataset(variant="medium", normalize_states=True):
    """Convenience function to load a Hopper dataset by variant name."""
    return HopperDataset(variant=variant, normalize_states=normalize_states)
