"""
Training script for Hopper offline RL experiments.

Usage:
    python -m hopper.train --algo cql --variant medium
    python -m hopper.train --algo iql --variant medium-expert --seed 42
    python -m hopper.train --algo bc --variant expert --total_steps 500000
"""
...
def build_agent(algo, device, **override_kwargs):
    """Build an agent with Hopper-tuned defaults, overridable by kwargs."""
...
def train(
    ...
    eval_episodes=10,
    save_freq=100_000,
    output_dir="./results/hopper",
    device="auto",
    **algo_kwargs,
):
    """Train an offline RL agent on a Hopper D4RL dataset."""
    np.random.seed(seed)
    ...
    print(f"\nLoading Hopper '{variant}' dataset...")
    ...
    print(f"\nTraining {algo.upper()} for {total_steps:,} steps on hopper-{variant}...")
    best_score = -np.inf

    for step in tqdm(range(total_steps), desc=f"hopper-{variant}/{algo}"):
    ...
    with open(os.path.join(save_dir, "final_metrics.txt"), "w", encoding="utf-8") as f:
        ...
        f.write(f"Environment: hopper-{variant}\n")
        f.write(f"Seed: {seed}\n")
...
def main():
    parser = argparse.ArgumentParser(description="Train offline RL on Hopper-v2")
    parser.add_argument(
        "--algo",
        type=str,
        default="cql",
    ...
    parser.add_argument("--output_dir", type=str, default="./results/hopper")
    parser.add_argument(
