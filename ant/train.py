"""
Training script for Ant offline RL experiments.

Usage:
    python -m ant.train --algo cql --variant medium
    python -m ant.train --algo iql --variant medium-expert --seed 42
    python -m ant.train --algo bc --variant expert --total_steps 500000
"""
...
from ant.dataset import load_ant_dataset
...
def build_agent(algo, device, **override_kwargs):
    """Build an agent with Ant-tuned defaults, overridable by kwargs."""
...
def train(
    ...
    eval_episodes=10,
    save_freq=100_000,
    output_dir="./results/ant",
    device="auto",
    **algo_kwargs,
):
    """Train an offline RL agent on an Ant D4RL dataset."""
    np.random.seed(seed)
    ...
    print(f"\nLoading Ant '{variant}' dataset...")
    dataset = load_ant_dataset(variant=variant, normalize_states=True)
    ...
    print(f"\nTraining {algo.upper()} for {total_steps:,} steps on ant-{variant}...")
    best_score = -np.inf

    for step in tqdm(range(total_steps), desc=f"ant-{variant}/{algo}"):
    ...
    with open(os.path.join(save_dir, "final_metrics.txt"), "w", encoding="utf-8") as f:
        ...
        f.write(f"Environment: ant-{variant}\n")
        f.write(f"Seed: {seed}\n")
...
def main():
    parser = argparse.ArgumentParser(description="Train offline RL on Ant-v2")
    parser.add_argument(
        "--algo",
        type=str,
        default="cql",
    ...
    parser.add_argument("--output_dir", type=str, default="./results/ant")
    parser.add_argument(
