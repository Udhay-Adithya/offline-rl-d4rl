"""
Evaluation script for trained Ant offline RL models.

Usage:
    python -m ant.evaluate --algo cql --variant medium \
        --model_path ./results/ant/cql_medium_seed0/best_model.pt
    python -m ant.evaluate --algo iql --variant medium-expert \
...
def evaluate(
    ...
    """Evaluate a trained Ant model across multiple seeds."""
    ...
    print("\n" + "=" * 60)
    print(f"Ant-{variant} | {algo.upper()} | {len(seeds)} eval seeds")
...
def main():
    parser = argparse.ArgumentParser(
        description="Evaluate a trained offline RL model on Ant-v2"
    )
    parser.add_argument(
        "--algo",
        type=str,
